...
`examples/quickstart.py` is a complete version of this snippet; `data/examples/*.json` contain the same information in plain JSON for inspection without any tooling.
...
```bash
python scripts/validate_dataset.py --data data                      # every camera (a few minutes for `full`)
python scripts/validate_dataset.py --data data --max-cameras 2000 --regenerate 5
```

The validator re-projects the stored ground truth for every camera and compares it with `points_2d_clean`, `point_indices` and `depths` (10⁻⁹ px), verifies rotations, camera centres, `K` structure and the FOV/focal relation, checks bounds and depths, counts outliers, bounds the largest inlier deviation with a sample-size-aware Gaussian tail bound (`√(2 ln(N/10⁻⁹))` σ — a fixed 6σ limit would fail by construction on a dataset with 10⁸ correspondences) and tests the inlier noise RMS against σ within eight standard errors, checks quantization, checks that swapped observations are a permutation and uniform outliers lie inside the image, checks planarity labels, compares the manifest with the HDF5 attributes and regenerates `--regenerate` random scenes to compare them bit-for-bit. It writes `data/metadata/validation_report.json` and exits with status 1 on any failure.
...
| `multiview` | K, distortion and one pose per view from all views sharing an intrinsic set — the classic multi-view calibration problem | raw pixels | outlier-free, non-quantized, non-planar scenes; rigs with ≥ 3 views |
...
* `--num-points 4,6,8,12,20,50,100,500` sets the subset sizes of the sweep; `--max-iters`, `--confidence`, `--threshold` control the robust estimators; `--seed` fixes the subset selection and the RANSAC seeds.
...
* **Noise.** Errors grow linearly with σ: with σ = 2 px and ≈ 1 000 points the median rotation error of the LM-refined solvers is ≈ 0.03 deg (DLT + LM 0.031, EPnP + LM 0.032, iterative 0.030, SQPnP 0.033); the linear DLT alone is three times worse (0.087). Integer-pixel quantization without noise costs 0.004–0.007 deg for every solver except the linear DLT (0.013), the same as σ ≈ 0.3 px, as expected from the 0.289 px standard deviation of rounding.
* **Outliers.** Non-robust solvers keep 0–60 % success at 5 % outliers and ≈ 0 % from 20 % on — that is what the outlier conditions measure. The from-scratch `ransac_p3p` (3-point hypotheses, LM re-fit) and MAGSAC++ keep 100 % success up to 80 % outliers and fall to 7 % and 29 % at 95 %, where 2 000 iterations are no longer enough (at 5 % inliers the expected number of 3-point samples for one all-inlier draw is 8 000). `cv_ransac_epnp` draws 5-point samples and already drops to 43 % at 80 % outliers, `cv_ransac_ap3p` (4-point samples) to 93 % — exactly the (1 − ε)ᵐ ordering. Swapped and uniform outliers are equally hard for every robust solver.
* **Planar scenes.** The DLT variants are reported as *degenerate* on every `planar_single` sample (118 of 600 solves) and IPPE, SQPnP and the iterative solver handle planes without any loss (0.011 deg median over the outlier-free conditions). OpenCV's EPnP, however, is numerically unreliable on coplanar points (54 % success on outlier-free planar samples, 0.19 deg median), and because `solvePnPRansac` always re-estimates the final pose with EPnP, all three `cv_ransac_*` variants inherit that weakness on planar scenes (0.14–0.36 deg median versus 0.004–0.019 deg on the other scene types) while `ransac_p3p` (0.006 deg) and MAGSAC++ (0.009 deg) do not.
* **Few points.** With n = 4, SQPnP succeeds on 98 % of the subsets (0.15 deg median), the minimal P3P / AP3P solvers on 91 % (0.3 deg), IPPE on 86 %; EPnP is not a minimal solver and reaches only 46 % (4.7 deg). From n ≈ 8 on, all LM-refined solvers converge to the same accuracy.
* **Fields of view.** For calibrated PnP the FOV barely matters to any solver but the linear DLT (0.003–0.017 deg medians in every class, IPPE on planes up to 0.022); only the DLT degrades on telephoto views (0.048 deg versus 0.015 deg at normal FOV). For anything that estimates intrinsics, narrow fields of view are the ill-conditioned case: the principal point becomes nearly unobservable and multi-view calibration returns principal-point errors of 50–85 px on telephoto rigs (both OpenCV and the bundle adjustment) although the reprojection error stays small.
* **Calibration.** The single-view DLT recovers the intrinsics of pinhole cameras to 0.04 % focal error, and shows the expected systematic bias on distorted cameras (1 % for Brown–Conrady, 5 % for Kannala–Brandt, 8.5 % for the fisheye class) because it has no distortion model. In multi-view calibration the from-scratch bundle adjustment matches OpenCV's `calibrateCamera` on Brown–Conrady rigs (0.0069 % focal error for both), is slightly better on pinhole rigs (0.005 % versus 0.018 %) and, unlike `cv2.fisheye.calibrate` — whose extrinsic initialisation assumes a planar target — converges on non-planar Kannala–Brandt rigs (0.03 % versus 6.5 % focal error, 100 % versus 40 % success).
...
It walks the repository and removes `__pycache__/`, `*.pyc`, `*.pyo`, `.pytest_cache/`, `.mypy_cache/`, `.ruff_cache/`, `.ipynb_checkpoints/`, `.coverage`, `htmlcov/`, `*.egg-info/`, `build/` and `dist/`, then prints what it removed and how much space it freed. It matches those names exactly, so it can never touch a dataset.
...
* **Planar inputs to OpenCV.** Coplanar points are expressed in their own plane frame ($z = 0$ exactly) before calling OpenCV, because OpenCV's EPnP is unstable when points are coplanar only up to floating-point rounding (a rotated plane) and its IPPE mishandles axis-aligned offset planes. The pose is mapped back exactly; the remaining planar failures of EPnP are properties of OpenCV's implementation and are reported as such.
