...
def run_pnp_benchmark(data_dir, manifest_subset: pd.DataFrame, solver_names: Optional[Sequence[str]] = None,
    ...
    with SampleReader(data_dir) as reader:
        for _, row in iterator:
            ...
            sub_rng = np.random.default_rng([seed, int(row["scene_id"]), int(row["camera_id"]), int(row["condition_id"])])
            for setting in num_points:
                if setting == "all":
                    ...
                    if int(setting) > m:
                        continue
                    idx = np.sort(sub_rng.choice(m, int(setting), replace=False))
                X = X_all[idx]
                uv = uv_und[idx]
                ...
                for spec in specs:
                    ...
                    try:
                        est = spec.fn(X, uv, intr.K, threshold=thr, max_iters=max_iters, confidence=confidence,
                                      seed=int(sub_rng.integers(2**31 - 1)))
            ...
            for method in methods:
                ...
                try:
                    if method == "opencv":
                        ...
                        est = calibrate_multiview_ba(views, (intr.width, intr.height), K0, model)
                except Exception as exc:  # a solver crash is recorded, never fatal
                ...
                est_model = calib_model if method == "opencv" else model
                est_dist = np.asarray(est.dist) if est_model != "pinhole" else np.zeros(5)
